<template>
  <div class="p-4 px-0">
    ...
    <form class="space-y-4">
      ...
      <div class="flex items-center space-x-4">
        <label for="stock" class="text-lg mb-1 w-32">AvailableQuantity</label>
        <input type="number" id="stock" v-model="newVariant.availableQuantity" class="flex w-1/2 p-2 border rounded-md">
      </div>

      <div class="flex items-center space-x-4">
        <label for="status" class="text-lg w-32">Primary</label>
        <label class="relative inline-flex items-center cursor-pointer">
          <input type="checkbox" v-model="newVariant.isDefault" id="isDefault" class="sr-only peer" />
          <div
    ...
    <div class="mt-4 hidden">
      ...
      <ul>
        <li v-for="(variant, index) in variants" :key="index" class="mb-2 p-2 rounded-md border shadow-sm variant-item flex gap-4 px-3">
          ...
          <span class="block text-lg"><strong>Stock:</strong> {{ variant.availableQuantity }}</span>
          <span class="block text-lg"><strong>Stock:</strong> {{ variant.isDefault }}</span>
...
const addNewVariant = () => {
  if (newVariant.size && newVariant.regularPrice && newVariant.price && newVariant.stock && newVariant.availableQuantity && newVariant.isDefault) {
